...
def get_k_details(k_link):
    ...
    kompetenz_subgroup_code = soup.find(class_='two columns htacode alpha').text #1 attr
    ...
    for i in standard_table_row:
        ...
        for num in range(1,5):
            try:
               zyk = i.find(class_='tooltip one column komp_cell marker_z'+str(num)).get('title')
               zyklus.append(int(re.findall(r'\d+', zyk)[0]))
            except:
                pass

        try:
            komp_code.append(i.find(class_='tooltip one column komp_cell kompetenz_lit').get('title'))
            querverweis.append(i.find(class_='querv').find(class_='tooltip').get('title'))
            querverweis_link.append(i.find(class_='querv').find(class_='tooltip').get('href'))
        except:
    ...
    kompetenz_text = soup.find_all(class_='eight columns komp_cell kompetenz_text')
    #remove breaks and possible whitespaces 
    kompetenz_text = [k.text.strip('\n').strip(' ').replace('z.B.', 'zum Beispiel') for k in kompetenz_text]
